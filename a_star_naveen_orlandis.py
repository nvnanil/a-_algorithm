...
out = cv2.VideoWriter('project_3.avi', fourcc, 30.0, (600, 250))
...
def w_space(max_x,max_y, clear):
    ...
    #Lower rectangle
        if y>=(100-clear) and y<=(150+clear) and x>=0 and x<=(100+clear):
            ...
            obstacle_space.append((x,y))
        elif (y >= (460 - clear)) and (x >= (25 - clear)) and (x <= (225 + clear)) and ((-2*y + x) >= -895 + clear) and ((2*y + x) <= 1145 - clear): #Triangle
            obstacle_space.append((x,y))
        elif (y >= (235 - clear)) and (y <= (365 + clear)) and ((y + 2*x) >= 390 - clear) and ((y - 2*x) <= 210 + clear) and ((y - 2*x) >= -130 + clear) and ((y + 2*x) <= 710 + clear):
            obstacle_space.append((x,y))
    #Puffing the walls
...
while ip:
    ...
    obstacles = w_space(width,height,clearance)
